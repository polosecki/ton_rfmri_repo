...
outlier_csv = '/home/CHDI/polo/half_baked_data/TON_rsfMRI/outlier_runs.csv'
...
def get_sdmt_slope(subject):
    ...
    sdmt = np.array([subject.pheno_df.ix[vn]['SDMT'] for vn in vis_names])
    try:
...
for name in regress_names:
    ...
    for s_i, s_name in enumerate(subjs_in_TONf):
        if first_visit[s_i] != 0:
            first_vect[s_i] = subjects_dict[s_name].pheno_df.ix['visit' + str(first_visit[s_i]) + '_ton'][name]
        if last_visit[s_i] != 0:
            last_vect[s_i] = subjects_dict[s_name].pheno_df.ix['visit' + str(last_visit[s_i]) + '_ton'][name]
...
dict_for_df = {'subject_IDs': subjs_in_TONf[ok_subjs],
               ...
               'age': age[ok_subjs],
               'sex': sex[ok_subjs]}
#               'first_yto': first_yto[ok_subjs],
#               'last_yto': last_yto[ok_subjs]}
...
#    mname = ('/home/CHDI/polo/code/MLtool/TON_resting_classification/proc_data/'
...
#             'TON_Resting_State_bct_degrees_log_norm_median.mat')
    ...
    dict_for_df = {'subject_IDs': subjs_in_TONf[in_Q1],
                   'group': gr_num[in_Q1],
                   'age': age,
                   'sex': sex}
    Q1df = pd.DataFrame(dict_for_df).set_index('subject_IDs').sort_index()
